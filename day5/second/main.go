...
func main() {
	...
	for _, el := range in {
		...
		if x1 == x2 {
			...
			// diagonal lines are always 45 degrees, so the num of steps in both
			// directions is equal, so max can be calculated based on one dir
			// also, determine the direction for the X axis
			if x1 > x2 {
				max = x1 - x2
				xDir = -1
			} else {
			...
			// determine the direction for the Y axis
			if y1 > y2 {
				yDir = -1
			} else {
				yDir = 1
			...
			for i := 0; i <= max; i++ {
				diag[y1+(i*yDir)][x1+(i*xDir)]++

				if diag[y1+(i*yDir)][x1+(i*xDir)] == 2 {
					// we've got overlapping lines, but only count first overlap
					overlaps++
				}
			}
